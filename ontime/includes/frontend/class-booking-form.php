...
final class OnTime_Frontend_Booking_Form {
	...
	public function maybe_enqueue() {
		...
		wp_localize_script( 'ontime-frontend', 'OnTimeData', array(
			...
			'i18n'    => array(
				...
				'redirecting'   => __( 'در حال انتقال به درگاه پرداخت...', 'ontime' ),
			),
		) );
	}
	...
	public function ajax_submit_booking() {
		...
		$summary  = array(
			...
			'price' => $calendar->to_persian_digits( (string) $price ),
		);

		if ( $price > 0 ) {
			$handler = OnTime_Payment_Handler::instance();
			$result  = $handler->process_payment( $appointment_id, $price );
			...
			wp_send_json_success( array(
				'message'      => __( 'در حال انتقال به درگاه پرداخت...', 'ontime' ),
				'redirect_url' => $result,
				'appointment'  => $summary,
			) );
		}
